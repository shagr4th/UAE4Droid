...
void gui_handle_events (void)
{
	...
	// on-screen button + up - change screen mode
	if(keystate[SDLK_F13] && keystate[SDLK_UP])
	{
		mainMenu_displayHires=!mainMenu_displayHires;
		update_display();
	}
	// on-screen button + up - change screen mode
	if(keystate[SDLK_F14] && keystate[SDLK_UP])
	{
	...
	// on-screen button + down - change frameskip
	if(keystate[SDLK_F14] && keystate[SDLK_DOWN])
	{
	...
	// on-screen button + left - save savestate
	if(keystate[SDLK_F14] && keystate[SDLK_LEFT])
	{	
	  keystate[SDLK_s]=0;
	  savestate_state = STATE_DOSAVE;
	}
	// on-screen button + right - restore savestate
	if(keystate[SDLK_F14] && keystate[SDLK_RIGHT])
	{
		extern char *savestate_filename;
		...
		if(f)
		{
			...
			// on-screen button - left mouse click
			...
			// on-screen button - right mouse click
...
if(!vkbd_mode)
{
	...
	if(triggerL && triggerR)
	{
		...
		{
			SetPresetMode(mainMenu_displayHires ? 10 : 0);
			update_display();
		}
		//2
		else if(keystate[SDLK_2])
		{
			SetPresetMode(mainMenu_displayHires ? 11 : 1);
			update_display();
		}
		//3
		else if(keystate[SDLK_3])
		{
			SetPresetMode(mainMenu_displayHires ? 12 : 2);
			update_display();
		}
		//4
		else if(keystate[SDLK_4])
		{
			SetPresetMode(mainMenu_displayHires ? 13 : 3);
			update_display();
		}
		//5
		else if(keystate[SDLK_5])
		{
			SetPresetMode(mainMenu_displayHires ? 14 : 4);
			update_display();
		}
		//6
		else if(keystate[SDLK_6])
		{
			SetPresetMode(mainMenu_displayHires ? 15 : 5);
			update_display();
		}
		//7
		else if(keystate[SDLK_7])
		{
			SetPresetMode(mainMenu_displayHires ? 16 : 6);
			update_display();
		}
		//8
		else if(keystate[SDLK_8])
		{
			SetPresetMode(mainMenu_displayHires ? 17 : 7);
			update_display();
		}
		//9
