...
char *kickstarts_rom_names[] = { "kick12.rom\0", "kick13.rom\0", "kick20.rom\0", "kick31.rom\0" };
...
//    addr -= chipmem_start /*& chipmem_mask*/;
   ...
   if (TraceGetMode() > 0)
      TraceLog (
         ...
         addr,
         "Rw",
         swab_w(do_get_mem_word ((uae_u16 *)m)),
   ...
   if (TraceGetMode() > 0)
      TraceLog (
         ...
         addr + 2,
         "Rw",
         swab_w(do_get_mem_word ((uae_u16 *)m + 1)),
         swab_l(do_get_mem_long ((uae_u32 *)((uae_u32)(m + 1) & ~3)))
      );
#endif
...
uae_u32 REGPARAM2 kickmem_lget (uaecptr addr)
{
   ...
   if (TraceGetMode() > 0)
      TraceLog (
         ...
         addr + 2,
         "Rw",
         swab_w(do_get_mem_word (m + 1)),
         swab_l(do_get_mem_long ((uae_u32 *)((uae_u32)(m + 1) & ~3)))
      );
#endif
   return (swab_w(do_get_mem_word(m)) << 16) |
          (swab_w(do_get_mem_word(m + 1)));
}
...
#endif
         m = (uae_u16 *)(kickmemory + addr);
         do_put_mem_word(m, swab_w(l >> 16));
         do_put_mem_word(m + 1, swab_w(l));
#ifdef DEBUG_TRACE
         if (TraceGetMode() > 0)
            TraceLog (
               TRACE_MODE_ADDRESS,
