...
static unsigned int active_line UAE4ALL_ALIGN;
static char line_changes_sprites;
static char *line_drawn;
static char linestate[(MAXVPOS + 1)*2 + 32];
...
static void pfield_draw_line_1 (); 
...
static __inline__ void count_frame (void)
{
	...
	// limiter..
#ifdef WITH_TESTMODE
	if ((ahora+partida)<proximo_frameskip && !no_limiter)
#else
...
static /*__inline__*/ void pfield_draw_line_1 () {
   pfield_doline (active_line);
...
static __inline__ void pfield_draw_line (int lineno, int gfx_ypos, int follow_ypos)
{
   ...
   xlinebuffer = row_map[gfx_ypos];
	 if(mainMenu_displayHires)
	 		xlinebuffer -= (LINETOSCR_X_ADJUST_BYTES + moveX) << 1;
	 else
...
static _INLINE_ void draw_status_line (int line)
{
    ...
    if(mainMenu_displayHires)
    	gfxvid_width = GFXVIDINFO_WIDTH * 2;
    else
    	gfxvid_width = GFXVIDINFO_WIDTH;
...
#ifdef WITH_TESTMODE
	x+=100 - (TD_WIDTH*(mainMenu_drives-1)) - 2*TD_WIDTH;
#else
  ...
  uae4all_memclr (xlinebuffer + (x - 4) * GFXVIDINFO_PIXBYTES, (gfxvid_width - x + 4) * GFXVIDINFO_PIXBYTES);
	...
	if (y >= TD_PADY && y - TD_PADY < TD_NUM_HEIGHT) 
	{
		if(test_mode)
		{
			write_tdletter(x, y - TD_PADY, 'T');
			write_tdletter(x + TD_NUM_WIDTH, y - TD_PADY, 'S');
			write_tdletter(x + 2*TD_NUM_WIDTH, y - TD_PADY, 'T');
		}
	}
	x+=TD_WIDTH;
#endif
...
static _INLINE_ void finish_drawing_frame (void)
{
	...
	for (i = 0; i < max_ypos_thisframe; i++) {
		int where,i1;
		active_line = i + thisframe_y_adjust_real;

		if (linestate[active_line] == LINE_UNDECIDED)
			break;
		...
		pfield_draw_line (active_line, where, amiga2aspect_line_map[i1 + 1]);
	}
