...
extern char line_changes[2 * (MAXVPOS + 1) + 1];

#define SET_LINE_CHANGE(LINENO, CHANGE) \
   { line_changes[LINENO] = CHANGE; }
#define MARK_LINE_CHANGED(LINENO) \
   { line_changes[LINENO] = 1; }
#ifdef __WINS__
   /* For debugging purposes, menu gfx is not cleared */ 
   #define MARK_ALL_LINES_CHANGED
#else
   #define MARK_ALL_LINES_CHANGED \
   { \
      for (int i=0; i<=(2 * (MAXVPOS + 1)); i++) \
         line_changes[i] = 1; \
   }
#endif
