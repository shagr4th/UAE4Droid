...
// 0 nn 110nnn - An + Disp + D/An
//
// 68020+ (full extension word), if s==1, An register is suppressed (and some modes make no sense?):
// 1 s 0 0n 0 001 - [An + Xn]
// 1 s 0 0n 0 01n - [An + Xn] + od
// 1 s 0 1n 0 001 - [bd + An + Xn]
// 1 s 0 1n 0 01n - [bd + An + Xn] + od
// 1 s 0 0n 0 101 - [An] + Xn
// 1 s 0 0n 0 11n - [An] + Xn + od
// 1 s 0 1n 0 101 - [bd + An] + Xn
// 1 s 0 1n 0 11n - [bd + An] + Xn + od
// 1 0 1 0n 0 001 - [An]
// 1 s 1 1n 0 01n - [bd + An]
// 1 s 1 0n 0 01n - [An] + od
// 1 s 1 1n 0 01n - [bd + An] + od
...
{ \
    ...
    if (ext & 0x0100) { \
        ...
        else if (ext & 0x30 == 0x30) { \
            FETCH_LONG(disp); \
        } \
        adr += disp; \
        \
        /* Indirect memory address */ \
        /* (only if outer displacement is used) */ \
        if (ext & 0x02) { \
            /* Preindexed */ \
            if (ext & 0x04 == 0) { \
                adr += index; \
            ...
            /* Indexed (pre and post have the same effect) */ \
            adr += index; \
        \
        /* Outer displacement */ \
        if (ext & 0x03 == 0x02) { \
