...
//   /* Keep trace flags (ex. STOP instruction) */
//   if (flag_T)
//      m68kcontext.execinfo |= M68K_EMULATE_TRACE/*|M68K_DO_TRACE*/;
   CHECK_INT_TO_JUMP(20)
...
OPCODE(0x8148)
{
   ...
   FETCH_WORD(src)
   adr = AREG((Opcode /*>> 0*/) & 7) - areg_byteinc[(Opcode /*>> 0*/) & 7];
   PRE_IO
   READ_BYTE_F(adr, dst)
   adr -= areg_byteinc[(Opcode /*>> 0*/) & 7];
   READ_BYTE_F(adr, res);
   AREG((Opcode /*>> 0*/) & 7) = adr;
   dst = (dst | (res << 8)) + src;
   dst = ((dst >> 4) & 0xF0) | (dst & 0x0F);
   adr = AREG((Opcode >> 9) & 7) - areg_byteinc[(Opcode >> 9) & 7];
   AREG((Opcode >> 9) & 7) = adr;
